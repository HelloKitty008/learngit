...
#数据库线程
def DB_thread(PLSNO,ActNum,SESign,BreakSign,BreakIma):
    db.ExecUpdata(PLSNO,ActNum,SESign,BreakSign,BreakIma)
    ...
    while Thread_sign:
        ...
        if sign==0 and buf!=buf1:
            ...
            #解析json
                ...
                t=threading.Thread(target=DB_thread,args=(PLSNO,ActNum,SESign,BreakSign,BreakIma))
                t.start()
            except:
                #print('json error')
                pass
    ...
    while True:
        ...
        while True:
            ...
            if Type=='r':
                print('r')
                #print(type(client))
                #client.sendall(bytes(PLSNO,encoding="utf-8"))
                #conn.sendall(bytes('True',encoding="utf-8"))
